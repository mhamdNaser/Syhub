function initVariantSwitcher() {
  ...
  if (scriptTag) {
    ...
    console.warn("⚠️ not available script[type='application/json'] داخل variant-selector");
    return;
  }
  ...
  function updateVariant(optionValues) {
    ...
    document.querySelectorAll('[class^="thumbnail-container_"]').forEach(thumb => {
    ...
    if (thumbnailContainer) {
      ...
      if (window.thumbnailsSwiper) {
        ...
        if (slideIndex >= 0) thumbnailsSwiper.slideTo(slideIndex);
      }
    }
  ...
  colorButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll('.color-swatch').forEach(b => b.classList.remove('border', 'border-[#c42764]'));
  ...
  document.querySelectorAll('[class^="thumbnail-container_"]').forEach(btn => {
    btn.addEventListener("click", () => {
      const mediaId = parseInt(btn.className.match(/thumbnail-container_(\d+)/)[1]);
      const selectedVariant = variantData.find(v => v.featured_media?.id === mediaId);
      if (!selectedVariant) return;
...
// تحميل السكربت ديناميكيًا مع استدعاء الفانكشن
window.addEventListener('load', () => {
  const variantJs = document.createElement('script');
  variantJs.src = "{{ 'plugin-product-variant-switcher.js' | asset_url }}";
  variantJs.onload = initVariantSwitcher;
  document.body.appendChild(variantJs);
});
